...
# Configuration
TOKEN = '7487481982:AAHfBWIO_sUDoKtalQXSjATqoLPY_AcsS8U'
...
def handle_genkey(message):
    if message.from_user.id != ADMIN_USER_ID:
        bot.reply_to(message, "𝙊𝙉𝙇𝙔 𝙊𝙒𝙉𝙀𝙍 𝘿𝙈-> @TRUSTVIP_MOD0")
        return
...
def handle_attack(message):
    ...
    if not user_data or user_data['plan'] == 0:
        bot.reply_to(message, "𝘿𝙈-> @TRUSTVIP_MOD0")
        return
...
def send_welcome(message):
    ...
    markup.add(""*❄️ᴡᴇʟᴄᴏᴍᴇ ᴛᴏ ᴘʀᴇᴍɪᴜᴍ ᴅᴅᴏs ʙᴏᴛ, {user_name}! ᴛʜɪs ɪs ʜɪɢʜ ǫᴜᴀʟɪᴛʏ sᴇʀᴠᴇʀ ʙᴀsᴇᴅ ᴅᴅᴏs. ᴛᴏ ɢᴇᴛ ᴀᴄᴄᴇss.  *\n\n"
